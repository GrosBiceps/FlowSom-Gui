...
- [FlowSOM Analysis Pipeline Pro](#flowsom-analysis-pipeline-pro)
  ...
  - [Fonctionnalités](#fonctionnalités)
    ...
    - [Cartographie de populations](#cartographie-de-populations)
    - [Export](#export)
...
### Cartographie de populations

Service dédié (`PopulationMappingService`) qui compare les centroids des nodes SOM aux profils MFI de référence :

| Méthode | Description |
|---|---|
| **v3 — Euclidien** | Assignation par distance euclidienne (normalisée range) |
| **v5 — Cosinus + Prior** | Distance cosinus pondérée par prior log10³ sur la taille attendue des populations (méthode ELN recommandée) |
| **Seuil inconnu** | Détection automatique par Otsu, percentile ou mean±σ |
| **Stats Mahalanobis/KNN** | Calcul de densité de population par node pour validation |

Les résultats de cartographie sont intégrés dans le `PipelineResult` et exportés automatiquement.

### Export

- Fichiers **FCS** avec colonnes de clustering ajoutées (compatible Kaluza)
- **CSV** : cellules complètes, statistiques par cluster, MFI par marqueur/metacluster, récapitulatif par fichier
- **JSON** : log de gating horodaté, métadonnées complètes du run, configuration
- **Graphiques Matplotlib** : heatmap MFI, distribution des metaclusters, UMAP, plots de gating
- **Graphiques Plotly** : Sankey diagrams de gating (global + par fichier), dashboard interactif, MST interactif, SOM grid
- **Rapport HTML** : rapport interactif auto-contenu (`analysis_report.html`) embarquant tous les graphiques
...
│   │
│   ├── visualization/        # Graphiques matplotlib + Plotly (thème sombre)
│   │   ├── plot_helpers.py   # Utilitaires de mise en forme
│   │   ├── gating_plots.py   # Plots de pré-gating + Sankey + dashboard interactif
│   │   ├── flowsom_plots.py  # Heatmap MFI, UMAP, MST statique + Plotly, SOM grid
│   │   ├── html_report.py    # Rapport HTML interactif auto-contenu
│   │   └── population_viz.py # Blast heatmap, radar, barres, heatmap comparative
│   │
│   ├── analysis/             # Analyses biologiques
│   │   ├── population_mapping.py  # Assignation populations → nodes (v3 euclidean, v5 cosine+prior)
...
│   │   ├── export_service.py             # ExportService (FCS, CSV, plots, JSON, HTML)
│   │   └── population_mapping_service.py # PopulationMappingService (v3/v5, blastes, viz)
│   │
│   └── pipeline/
...
auto_clustering:
  enabled:                  false
  min_clusters:             5
  max_clusters:             20    # max 35 si recherche large
  n_bootstrap:              10
...
markers:
  ...
  keep_area_only:     true      # ne garder que les canaux -A (Area)

downsampling:
  enabled:            false     # désactivé par défaut (activer si >50 k cellules/fichier)
  max_cells_per_file: 50000
  max_cells_total:    1000000

visualization:
  save_plots:   true
  umap_enabled: false           # UMAP (désactivé par défaut, consommateur de mémoire)
  plot_format:  "png"           # png | pdf | svg
  dpi:          300
  figures:                      # paramètres fins par graphique (optionnel)
    heatmap:
      figsize: [16, 8]
      cmap: "magma"
    metacluster_dist:
...
population_mapping:
  enabled:                  true
  ref_mfi_dir:              "Data/Ref MFI"  # dossier des CSV de référence MFI NBM
  cache_dir:                "output/ref_mfi_parquet_cache"
  distance_percentile:      60
  include_scatter:          true
  normalization_method:     "range"         # range | zscore
  mapping_method:           "cosine_prior"  # v5 recommandé ELN 2022
  unknown_threshold_mode:   "auto_otsu"     # auto_otsu | percentile | mean_std
...
│   ├── gating_sankey.html                # Sankey interactif : flux cellulaire de gating
│   ├── gating_dashboard.html             # Dashboard interactif par fichier
...
│   ├── flowsom_umap.png                  # Projection UMAP (si umap_enabled: true)
│   ├── flowsom_mst_static.png            # MST FlowSOM (matplotlib statique)
│   ├── flowsom_mst_interactive.html      # MST FlowSOM (Plotly interactif)
│   ├── flowsom_som_grid.html             # Grille SOM (Plotly, colorée par metacluster)
│   ├── population_blast_heatmap.png      # Heatmap scores de blastes par node
│   ├── population_blast_radar.html       # Radar chart des profils de blastes (Plotly)
│   └── population_comparative_heatmap.png# Heatmap comparative nodes vs références MFI
│
├── other/
...
│
└── analysis_report.html                  # Rapport HTML interactif auto-contenu (tous graphiques)
```
...
|---|---|---|
| `FlowSOM_cluster` | int | Index du node SOM (0 – xdim×ydim−1) |
| `FlowSOM_metacluster` | int | Index du métacluster |
| `condition` | str | `"Sain"` ou `"Pathologique"` |
| `file_origin` | str | Nom du fichier FCS source |
| `Condition_Num` | int | Encodage numérique de la condition (0/1) |
...
| `plotly` | `pip install plotly` | Visualisations interactives (Sankey, MST, SOM, HTML report) |
...
│   ├── visualization/       # Graphiques matplotlib + Plotly + HTML
│   │   ├── plot_helpers.py
│   │   ├── gating_plots.py
│   │   ├── flowsom_plots.py
│   │   ├── html_report.py   # Rapport HTML auto-contenu
│   │   └── population_viz.py# Blast heatmap/radar + heatmap comparative
│   ├── analysis/            # Analyses biologiques (MRD, blastes, stats)
│   ├── services/            # Orchestration (preprocessing, clustering, export)
│   │   ├── preprocessing_service.py
│   │   ├── clustering_service.py
│   │   ├── export_service.py
│   │   └── population_mapping_service.py  # Cartographie populations (v3/v5)
│   └── pipeline/
